import { type TailwindColors, twColors, type Color } from "./colors";
...
function findEquivalentTailwindClass(
	hexColor: string,
	...
	for (const [colorName, color] of Object.entries(tailwindColors)) {
		if (typeof color === "string") {
			...
			for (const [shade, shadeColor] of Object.entries(color)) {
				if (
					typeof shadeColor === "string" &&
					shadeColor.toLowerCase() === hexColor.toLowerCase()
				) {
		...
		if (typeof color === "string") {
			...
			for (const [shade, shadeColor] of Object.entries(color)) {
				...
				const distance = colorDistance(
					inputColor,
					typeof shadeColor === "string" ? hexToRgb(shadeColor) : shadeColor
				);
				if (distance < smallestDistance) {
					smallestDistance = distance;
					closestColorName = colorName;
...
export default function colorToTailwindClass(colorInput: string): string {
	const isTailwindColorName = Object.keys(twColors).includes(
	...
	const hexColor = !isColorHexValue(colorInput)
		? chroma(colorInput).hex()
		: colorInput;
